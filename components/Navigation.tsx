...
const Navigation: React.FC<NavigationProps> = ({ role, currentView, setView, onLogout, darkMode, toggleTheme }) => {
  ...
  return (
    <>
      ...
      <div className={`fixed top-4 bottom-4 right-4 z-[220] w-[calc(100vw-2rem)] sm:w-[450px] bg-white dark:bg-[#080d1a] shadow-2xl border border-slate-200 dark:border-white/10 transform transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] flex flex-col overflow-hidden ${isDrawerOpen ? 'translate-x-0' : 'translate-x-[calc(100%+2rem)]'}`}>
        ...
        <div className="flex-none p-8 flex items-center justify-between border-b border-slate-100 dark:border-white/5 bg-slate-50/30 dark:bg-white/[0.01]">
          <button
            onClick={() => setIsDrawerOpen(false)}
            className="p-4 hover:bg-rose-50 dark:hover:bg-rose-500/10 text-slate-400 hover:text-rose-500 transition-colors group"
          >
          ...
          <div className="flex items-center gap-4 cursor-pointer group text-right" onClick={() => { setView('dashboard'); setIsDrawerOpen(false); }}>
            ...
            <div className="p-3 bg-blue-500/10 dark:bg-cyan-500/10 text-blue-600 dark:text-cyan-400 shadow-sm">
              <Icons.Logo size={28} />
            </div>
          </div>
        </div>
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-end gap-6 px-10 py-6 transition-all duration-300 border-r-4 group relative
                     ${isActive
                      ? 'bg-blue-600 text-white shadow-[0_8px_30px_-5px_rgba(37,99,235,0.4)] dark:bg-cyan-500/10 dark:text-cyan-400 dark:border-cyan-400 border-blue-600'
        ...
        <div className="flex-none p-0 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-black/20">
          <div className="flex flex-col divide-y divide-slate-100 dark:divide-white/5 border-b border-slate-100 dark:border-white/5">
            ...
            <button
              ...
              className={`flex items-center justify-between px-10 py-5 transition-all hover:bg-white dark:hover:bg-white/5 ${currentView === 'profile' ? 'text-emerald-600 dark:text-emerald-400 bg-white dark:bg-emerald-500/10' : 'text-slate-500 dark:text-slate-400'}`}
            >
              <div className="flex items-center gap-2">
                {currentView === 'profile' && <div className="w-2 h-2 bg-current rounded-full"></div>}
              </div>
              <div className="flex items-center gap-3">
                <span className="font-bold text-[10px] uppercase tracking-widest">User Profile</span>
                <Icons.User size={18} />
              </div>
            </button>
            ...
            <button
              onClick={toggleTheme}
              className="flex items-center justify-between px-10 py-5 transition-all hover:bg-white dark:hover:bg-white/5 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
            >
              <div className="flex items-center gap-2"></div>
              <div className="flex items-center gap-3">
                <span className="font-bold text-[10px] uppercase tracking-widest">{darkMode ? 'Light Mode' : 'Dark Mode'}</span>
