  ...
  return (
    <div className={`fixed inset-0 z-[100] p-8 h-screen flex flex-col resolve-ui overflow-hidden transition-colors duration-500 ${isDark ? 'bg-black text-white' : 'bg-[#f8fdfe] text-[#1a365d]'}`}>
      ...
      <div className="relative flex-grow min-h-[400px] group">
        <div className={`absolute inset-0 rounded-[3.5rem] overflow-hidden shadow-2xl border transition-colors duration-500 ${isDark ? 'bg-[#050505] border-white/10' : 'bg-white border-slate-200'}`}>
              ...
              <div className={`px-6 py-4 rounded-[2rem] border backdrop-blur-xl flex items-center gap-4 ${isDark ? 'bg-black/50 border-white/10' : 'bg-white/50 border-slate-200'}`}>
                <div className="relative w-10 h-10 flex items-center justify-center">
                  <svg className="absolute inset-0 w-full h-full -rotate-90">
                    ...
                    <circle cx="20" cy="20" r="18" fill="none" stroke="#48c1cf" strokeWidth="3" strokeDasharray={113} strokeDashoffset={113 - (113 * (30 - timer)) / 30} className="transition-all duration-1000" />
                  </svg>
