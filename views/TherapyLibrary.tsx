              ...
              {cat.exercises.map((ex, i) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-6 rounded-[2rem] border transition-all group flex items-center justify-between ${isDone
                      ? 'opacity-60 cursor-not-allowed grayscale'
                      : 'hover:border-[#48c1cf] hover:bg-[#48c1cf]/5'
                      } ${darkMode ? 'border-white/5 bg-[#0a0a0a]' : 'border-slate-50 bg-slate-50'}`}
                  >
                    <div className="max-w-[80%]">
                      <p className="font-black text-base mb-1 text-[#1a365d] dark:text-white flex items-center gap-2">
                        {ex.name}
                        {isDone && <span className="text-[8px] px-2 py-0.5 bg-emerald-500/10 text-emerald-500 rounded-full font-black uppercase tracking-widest">Done</span>}
                      </p>
                    ...
                    <div className="text-[#48c1cf] opacity-0 group-hover:opacity-100 transition-opacity translate-x-2 group-hover:translate-x-0 duration-300">
                      {isDone ? (
                        <div className="p-2 bg-emerald-500/10 rounded-lg text-emerald-500 opacity-100">
