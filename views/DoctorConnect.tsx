      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {doctors.length === 0 ? (
          ...
          doctors.map((dr) => {
            ...
            return (
              <div key={dr.id} className="bg-white dark:bg-[#050505] p-10 rounded-[3rem] border border-white/5 shadow-lg group hover:border-[#48c1cf] transition-all flex flex-col items-start relative overflow-hidden">
                <div className="w-16 h-16 bg-[#f0f7f8] dark:bg-white/5 rounded-2xl flex items-center justify-center text-[#48c1cf] mb-8 border border-white/10 group-hover:bg-[#48c1cf] group-hover:text-white transition-all">
                  <Icons.User />
                </div>
                
                <div className="flex items-center gap-2.5 mb-2">
                  <h3 className="text-3xl font-black text-[#1a365d] dark:text-white tracking-tighter leading-none">{dr.name}</h3>
                  {dr.isVerified && (
                    <div className="flex-shrink-0 animate-in zoom-in-50 duration-500">
                      <Icons.Verified size={20} />
                    </div>
                  )}
                </div>
                
                <p className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-500 mb-10">Board Certified Neuro-Specialist</p>
